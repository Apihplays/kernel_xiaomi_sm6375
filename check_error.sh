...
analyze_errors() {
    ...
    while IFS= read -r line; do
        if [[ "$line" =~ " error:" || "$line" =~ " fatal error:" || "$line" =~ "undefined reference to" ]]; then
            # Flush previous error block before starting a new one
...
process_error_block() {
    ...
    elif grep -q "undefined symbol: __llvm_profile_instrument" <<< "${error_block[@]}"; then
        error_type="PGO运行时库缺失"
        suggestion="建议: build脚本中-fprofile-generate需要链接compiler-rt profile运行时库。检查build脚本是否自动检测libclang_rt.profile-aarch64.a，或设置PROFILE_GEN=0禁用PGO"
    elif grep -q "symbol not found:" <<< "${error_block[@]}"; then
        error_type="链接器符号缺失"
        suggestion="建议: ThinLTO可能将符号internal化。在LDFLAGS_vmlinux中添加-Wl,--export-dynamic可防止此问题（注意不要放在KBUILD_LDFLAGS中，否则会影响VDSO编译）。"
    elif grep -q "unknown argument.*export-dynamic" <<< "${error_block[@]}"; then
        error_type="链接器参数冲突"
        suggestion="建议: --export-dynamic被错误传递给VDSO编译。应将该参数从KBUILD_LDFLAGS移至LDFLAGS_vmlinux，仅用于vmlinux链接。"
    elif grep -q "undefined symbol:" <<< "${error_block[@]}"; then
        error_type="链接错误: 未定义符号"
        suggestion="建议: 检查是否缺少链接的库，或函数名是否拼写错误。"
    fi
