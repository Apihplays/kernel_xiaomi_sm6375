...
static int ksu_handle_event(struct fsnotify_group *group, struct inode *inode, u32 mask, const void *data, int data_type,
...
static int add_mark_on_inode(struct inode *inode, u32 mask, struct fsnotify_mark **out)
{
    ...
    if (fsnotify_add_inode_mark_locked(m, inode, 0)) {
        fsnotify_put_mark(m);
        return -EINVAL;
    }
    *out = m;
...
{
    if (wd->mark) {
        fsnotify_detach_mark(wd->mark);
        fsnotify_destroy_mark(wd->mark, g);
        fsnotify_free_mark(wd->mark);
        wd->mark = NULL;
    }
    if (wd->inode) {
        iput(wd->inode);
